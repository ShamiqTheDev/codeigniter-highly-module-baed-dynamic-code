    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            html += '<table class="listing_table table table-striped table-bordered">' +
                    ...
                    '<th scope="col">Sr#</th>' +
                    '<th scope="col">Treaty Particular Name</th>' +
...
$(document.body).on('click', ".delete_"+ $module +"_href", function () {
    $("#id").val($(this).attr("get_id"));
    $('#modaldemo5').modal('show');
});


$(".delete_"+$module).click(function () {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: $('#delete_'+ $module +'_popup').serialize(),
        dataType: 'json',
        headers: {
            'X-CSRF-Token': $('input[name="csrf_test_name"]').attr('value')
        },
